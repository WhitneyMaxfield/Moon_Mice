###############################################################
# CANOPY STRUCTURE & MOONLIGHT PIPELINE FOR 2025 DATA
#
# Purpose:
...
master_log <- PreAbs_complete_with_lat_lon
...
#do not run this more than once or R gets mad. 
...
#removing bucket 11 because the coordinates are written down wrong :)
buckets <- master_log |>
  filter(stationID != "BKT11") |>
  group_by(stationID) |>
  summarise(
...
# filtering Upper images ONLY 
# select variables wanted 
canopy_upper <- canopy_2025_clean |>
  filter(grepl("^Upper", Image)) |>
  select(
    BKT,
    Date,
    Image,
